...
# Original Categories
CLASSES = ['Pedestrian', 'Car', 'Cyclist', 'Van', 'Truck',  'Person_sitting',
      'Tram', 'Misc', 'DontCare']
...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        img, target = self.prepare(img, target, self.cat_ids)
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target, cat_ids):
        w, h = image.size
...
def build(image_set, args):
    ...
    PATHS = {
        "train": (root / 'data_tracking_image_2/training/image_02', root / "annotations" / f'tracking_{image_set}_half.json'),
        "val": (root / 'data_tracking_image_2/training/image_02', root / "annotations" / f'tracking_{image_set}_half.json'),
    }
